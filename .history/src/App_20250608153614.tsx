...
import { Code, Coffee, Zap, Heart, Star, Trophy, GitBranch, Bug, Layers, Palette, FileText, AlertCircle, CheckCircle, BarChart3 } from 'lucide-react';
...
const JSCodingPet = () => {
  ...
  const addDetailedActivity = (type: string, details: any = {}) => {
  ...
  const calculateExpGain = (type, details) => {
    const baseExp = {
      'code-js': 12,
      'code-ts': 15,
      'react-component': 25,
  ...
  const updateSpecificStats = (pet, type, details) => {
    const updates = { ...pet };
  ...
  const getMoodFromActivity = (type) => {
  ...
  const getMoodEmoji = () => {
    ...
    return moods[pet.mood] || '😊';
  };

  const getSkillLevel = (current, thresholds) => {
  ...
  function getDetailedActivityText(type, details) {
    const texts = {
      ...
      'testing': `Escribiste ${details.tests || 1} test${details.tests > 1 ? 's' : ''}`,
